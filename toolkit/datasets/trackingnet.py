...
class TrackingNetVideo(Video):
    ...
    def __init__(self, name, root, video_dir, init_rect, img_names, gt_rect, attr, load_img=False):
        super(TrackingNetVideo, self).__init__(name, root, video_dir, init_rect, img_names, gt_rect, attr, load_img)
    ...
    #                 self.pred_trajs[name] = [list(map(float, x.strip().split(','))) for x in f.readlines()]
